...
```bash
curl -fsSL https://raw.githubusercontent.com/UberMetroid/Sunrise/master/Thanatonaut/install.sh | bash
```
...
3. **Configuration Setup:** Initializes `~/.config/thanatonaut/` adhering to the XDG Base Directory specification.
4. **Safety Backup:** Backs up your original `steam_api64.dll` to `steam_api64_original.dll`.
5. **Path Linking:** Creates a symlink at `~/.local/bin/thanatonaut` so the binary is available globally.
6. **Desktop & systemd Services:** Installs a desktop shortcut at `~/Desktop/thanatonaut-server.desktop` and a user service for background execution.
...
To scan and index all Destiny 2 package archives into `~/.config/thanatonaut/cache/`:
```bash
thanatonaut index
```
...
Settings are stored at `~/.config/thanatonaut/config.json`:
