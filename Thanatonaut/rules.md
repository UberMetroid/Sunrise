...
This document establishes the official engineering guidelines, RFC compliance mandates, AST naming conventions, user preferences, and coding standards for the Linux emulation suite of **Project Sunrise** (`Thanatonaut/`).
...
3. **Linux Desktop & System Specifications:**
   - **XDG Base Directory Specification:** Proper placement in `~/.config/thanatonaut/`, `~/.local/share/applications/`, and `~/.local/share/icons/`.
...
1. **Strict Subdirectory Confinement:**
   - All Linux-specific binaries, Rust crates, scripts, assets, Docker configs (`Thanatonaut/Dockerfile`, `Thanatonaut/docker-compose.yml`), and documentation must reside **100% inside the `Thanatonaut/` directory**.
...
2. **Artifact Locations:**
   - Configuration & Runtime Cache: `~/.config/thanatonaut/`
   - Global Binary: `~/.local/bin/thanatonaut`
...
1. **Continuous Version Bumps:**
   - Every modification, new feature, or bugfix must increment the version number across all three canonical locations:
     - `Thanatonaut/Cargo.toml` (`version = "x.y.z"`)
     - `Thanatonaut/src/lib.rs` (`pub const THANATONAUT_VERSION = "x.y.z";`)
     - `Thanatonaut/src/settings/config.rs` (`version: "x.y.z".to_string()`)

2. **GitHub Synchronization:**
   - Every update must be staged (`git add Thanatonaut/`), committed with a clear descriptive message, and pushed directly to `origin master`.
...
1. **Start Menu Exclusivity:**
   - Application shortcuts must be installed directly into `~/.local/share/applications/` (`destiny2-thanatonaut.desktop` and `thanatonaut-server.desktop`).
