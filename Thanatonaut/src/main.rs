...
use thanatonaut::crypto::hash::sha256_hex;
use thanatonaut::installer::config_setup::SunriseDirectories;
use thanatonaut::installer::depot_downloader::DepotDownloader;
use thanatonaut::installer::desktop_entry::DesktopIntegration;
use thanatonaut::installer::doctor::SunriseDoctor;
use thanatonaut::installer::ghost_narrative::*;
use thanatonaut::installer::mod_installer::ModInstaller;
use thanatonaut::installer::steam_locator::search_destiny2_installations;
use thanatonaut::installer::uninstaller::Uninstaller;
use thanatonaut::protocol::bap_frame::{BapFrame, BAP_MAGIC};
...
use thanatonaut::state::light_calculator::{calculate_base_light, GearSlots};
use thanatonaut::state::package_scanner::PackageIndex;
use thanatonaut::THANATONAUT_VERSION;

fn print_usage(prog: &str) {
    print_banner();
    println!("Project Sunrise // Linux Vanguard Emulation Suite (v{})", THANATONAUT_VERSION);
    ...
    println!("                                   Env: THANATONAUT_BIND_ADDRESS, THANATONAUT_PORT, THANATONAUT_UDP_BIND, THANATONAUT_UDP_PORT (default udp: 7778)");
...
fn run_install(args: &[String]) -> bool {
    ...
    if download_depots {
        let target = installations.first().map(|i| i.game_root.clone()).unwrap_or_else(|| {
            let home = thanatonaut::installer::steam_locator::get_home_dir();
    ...
    if let Ok(current_exe) = env::current_exe() {
        if install_desktop {
            ...
            log_ok("START MENU", "destiny2-thanatonaut.desktop -> Applications menu");
            log_ok("WRAPPER SCRIPT", "~/.local/bin/thanatonaut-game launcher created");
        }
        if install_server {
            let _ = DesktopIntegration::install_systemd_service(&current_exe);
            log_ok("SYSTEMD SERVICE", "thanatonaut.service (daemon-reloaded)");
        }
    }
...
fn run_self_tests() -> bool {
    ...
    let digest = sha256_hex(b"thanatonaut");
...
fn main() {
    ...
    match args[1].as_str() {
        ...
        "depot" | "download" => {
            let target = args.get(2).map(PathBuf::from).unwrap_or_else(|| {
                search_destiny2_installations().first().map(|i| i.game_root.clone()).unwrap_or_else(|| {
                    thanatonaut::installer::steam_locator::get_home_dir()
        ...
        "server" => {
            let env_addr = env::var("THANATONAUT_BIND_ADDRESS").or_else(|_| env::var("THANATONAUT_BIND_ADDR"))
                .unwrap_or_else(|_| "127.0.0.1".to_string());
            let env_port = env::var("THANATONAUT_PORT").ok().and_then(|p| p.parse::<u16>().ok()).unwrap_or(7777);
            let env_udp_bind = env::var("THANATONAUT_UDP_BIND").unwrap_or_else(|_| "127.0.0.1".to_string());
            let env_udp_port = env::var("THANATONAUT_UDP_PORT").ok().and_then(|p| p.parse::<u16>().ok()).unwrap_or(7778);
        ...
        "version" | "-v" | "--version" => { println!("thanatonaut v{}", THANATONAUT_VERSION); }
