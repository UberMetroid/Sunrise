...
impl DesktopIntegration {
    ...
    pub fn install_game_launcher_wrapper() -> Result<()> {
        ...
        let wrapper_path = bin_dir.join("thanatonaut-game");
...
if ! nc -z 127.0.0.1 7777 2>/dev/null && ! ss -tulpn | grep -q ":7777"; then
    ...
    if command -v systemctl &>/dev/null && systemctl --user is-enabled thanatonaut.service &>/dev/null; then
        systemctl --user start thanatonaut.service
    else
        nohup "$HOME/.local/bin/thanatonaut" server >/dev/null 2>&1 &
    fi
    sleep 1
fi
...
"#;
        ...
        fs::write(&wrapper_path, script)
            .map_err(|e| SunriseError::IoError(format!("Failed to write thanatonaut-game wrapper: {}", e)))?;
    ...
    pub fn install_desktop_entry(binary_path: impl AsRef<Path>) -> Result<()> {
        ...
        if desktop_dir.exists() {
            let _ = fs::remove_file(desktop_dir.join("thanatonaut-server.desktop"));
            let _ = fs::remove_file(desktop_dir.join("destiny2-thanatonaut.desktop"));
        }
        ...
        let game_content = format!(
            "[Desktop Entry]\n\
             ...
             Exec={}/.local/bin/thanatonaut-game\n\
             Icon=sunrise\n\
             Terminal=false\n\
             Type=Application\n\
             Categories=Game;\n",
        ...
        let server_menu = apps_dir.join("thanatonaut-server.desktop");
        let game_menu = apps_dir.join("destiny2-thanatonaut.desktop");
    ...
    pub fn install_systemd_service(binary_path: impl AsRef<Path>) -> Result<()> {
        ...
        let service_file = service_dir.join("thanatonaut.service");
        let content = format!(
            "[Unit]\n\
