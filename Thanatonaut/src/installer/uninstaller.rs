...
impl Uninstaller {
    ...
    pub fn remove_desktop_integration() -> Result<()> {
        ...
        let desktop_file = home.join("Desktop").join("thanatonaut-server.desktop");
        let game_desktop = home.join("Desktop").join("destiny2-thanatonaut.desktop");
        ...
        let menu_file = home
            ...
            .join("share")
            .join("applications")
            .join("thanatonaut-server.desktop");
        let game_menu = home
            .join(".local")
            .join("share")
            .join("applications")
            .join("destiny2-thanatonaut.desktop");
        if menu_file.exists() {
        ...
        let service_file = home
            ...
            .join("systemd")
            .join("user")
            .join("thanatonaut.service");
        if service_file.exists() {
            let _ = fs::remove_file(service_file);
        }

        // 4. Remove local binary link
        let bin_link = home.join(".local").join("bin").join("thanatonaut");
        if bin_link.exists() {
    ...
    pub fn purge_config_directory(purge_data: bool) -> Result<()> {
        if purge_data {
            let home = get_home_dir();
            let config_dir = home.join(".config").join("thanatonaut");
            if config_dir.exists() {
                fs::remove_dir_all(config_dir).map_err(|e| {
                    SunriseError::IoError(format!("Failed to purge ~/.config/thanatonaut: {}", e))
                })?;
            }
        }
        Ok(())
