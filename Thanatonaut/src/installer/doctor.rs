    ...
    pub fn check_status() {
        ...
        match TcpStream::connect("127.0.0.1:7777") {
            ...
            Err(_) => {
                ...
                ghost_dialogue("\"The server is offline. Start it with 'thanatonaut server' whenever you're ready.\"");
            }
        }
        println!();
    }
    ...
    pub fn run_diagnostics() -> bool {
        ...
        if let Some(first) = installations.first() {
            ...
            if backup_path.exists() {
                ...
                log_scan("CORE SAFEGUARD", "No backup DLL found (run 'thanatonaut install')");
            }
            ...
            if first.steam_api_dll.exists() {
                ...
                if orig_len > 0 && cur_len != orig_len {
                    ...
                    log_scan("PROXY HOOK", "Vanilla DLL active (run 'thanatonaut install')");
                }
            }
        ...
        // 4. Check ~/.config/thanatonaut Sandbox & Package Cache
        ...
        if dirs.config_dir.exists() && dirs.config_file.exists() {
            ...
            log_scan("SANDBOX CONFIG", "Missing ~/.config/thanatonaut/config.json");
            all_healthy = false;
        }
        ...
        if cache_index.exists() {
            ...
            log_info("PACKAGE INDEX", "Run 'thanatonaut index' to cache package manifest");
        }

        // 5. Check Start Menu Launchers & Icons
        let icon_file = home.join(".local/share/icons/hicolor/scalable/apps/sunrise.svg");
        let menu_game = home.join(".local/share/applications/destiny2-thanatonaut.desktop");
        let menu_server = home.join(".local/share/applications/thanatonaut-server.desktop");
        ...
        if menu_game.exists() && menu_server.exists() {
            ...
            log_scan("START MENU", "Launchers not installed (run 'thanatonaut install')");
        }
