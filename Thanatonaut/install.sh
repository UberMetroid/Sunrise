...
elif [ -n "$SCRIPT_DIR" ] && [ -f "$SCRIPT_DIR/Thanatonaut/Cargo.toml" ]; then
    BUILD_DIR="$SCRIPT_DIR/Thanatonaut"
    echo -e "  ${GREEN}[  OK  ]${RESET} ${WHITE}Root Workspace:${RESET} $BUILD_DIR"
else
    THANATONAUT_REPO="${THANATONAUT_REPO:-https://github.com/UberMetroid/Sunrise.git}"
    THANATONAUT_BRANCH="${THANATONAUT_BRANCH:-master}"
    CLONE_DIR="$HOME/.cache/sunrise-build"

    ghost_box "\"Transmitting beacon coordinates to $THANATONAUT_REPO... Pulling down the latest Vanguard emulation blueprints.\""
    echo -e "  ${YELLOW}[ SYNC ]${RESET} ${WHITE}Downloading repository:${RESET} $THANATONAUT_REPO ($THANATONAUT_BRANCH)"
    rm -rf "$CLONE_DIR"
    mkdir -p "$CLONE_DIR"
    git clone --depth 1 --branch "$THANATONAUT_BRANCH" "$THANATONAUT_REPO" \
        "$CLONE_DIR" --quiet
    if [ -d "$CLONE_DIR/Thanatonaut" ]; then
        BUILD_DIR="$CLONE_DIR/Thanatonaut"
...
mkdir -p "$LOCAL_BIN"
ln -sf "$BUILD_DIR/target/release/thanatonaut" "$LOCAL_BIN/thanatonaut"
echo -e "  ${GREEN}[  OK  ]${RESET} ${WHITE}Global Executable Linked:${RESET} $LOCAL_BIN/thanatonaut"
...
"$BUILD_DIR/target/release/thanatonaut" install

echo -e "${GREEN}=======================================================================${RESET}"
echo -e "${GREEN}  TRANSMAT STATUS: READY TO LAUNCH                                     ${RESET}"
echo -e "${GREEN}=======================================================================${RESET}"
echo -e "  Launch Terminal Server:   ${CYAN}thanatonaut server${RESET}"
echo -e "  Launch Background Daemon: ${CYAN}systemctl --user start sunrise${RESET}"
echo -e "  Launch Destiny 2 Wrapper: ${CYAN}thanatonaut-game${RESET}"
